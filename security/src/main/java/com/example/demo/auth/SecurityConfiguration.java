	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.httpBasic(HttpBasicConfigurer::disable) // 기본 설정 disable
			...
			.authorizeHttpRequests((authz) -> authz
					...
					.requestMatchers("/","/join","/autherror", "/login").permitAll()
					.requestMatchers("/auth").authenticated())
			// 로그인 폼 설정
			.formLogin((login) -> login
					.loginPage("/loginform") // 로그인 폼을 주는 요청 url
					.loginProcessingUrl("/login") // 로그인 처리 요청 url
					.usernameParameter("id") // id 입력 양식의 이름
					.passwordParameter("pwd") // 패스워드 입력 양식의 이름
					.defaultSuccessUrl("/", true).permitAll() // 성공 시 기본 이동 경로
					.successHandler(new MySuccessHandler()) // 성공 시 실행 될 핸들러 
					.failureHandler(new MyFailureHandler()) // 실패 시 실행 될 핸들러
					);
		return http.build();
	}
}
